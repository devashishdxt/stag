...
impl State {
    fn parse(&self) -> Result<(ChainId, U256, Identifier, String)> {
...
pub fn burn_form(props: &Props) -> Html {
                        ...
                        .await
                        {
                            Ok(()) => {
                                ...
                                notification.set(Some(NotificationData::success(
                                    "Successfully burnt tokens".to_string(),
                                )));
                            }
                            Err(err) => {
                                error!("Failed to burn tokens: {:?}", err);
                                notification.set(Some(NotificationData::error(
                                    "Failed to burn tokens".to_string(),
                                )));
                            }
                        }
                    });
    ...
    html! {
        <div class={classes!("p-6")}>
            <h2 class={classes!("text-2xl", "pb-6", "font-bold")}>{ "Burn Tokens" }</h2>
            <form class={classes!("pl-4")} onsubmit={on_submit}>
                <TextInput class={classes!("mb-4")} placeholder="Chain ID" value={ state.chain_id.clone() } />
                <TextInput class={classes!("mb-4")} placeholder="Amount of tokens to burn" value={ state.amount.clone() } />
                <TextInput class={classes!("mb-4")} placeholder="Denom of tokens to burn" value={ state.denom.clone() } />
...
async fn burn(
    ...
    stag.burn(chain_id, None, amount, denom, memo)
        .await
        .map(|_| ())
}
