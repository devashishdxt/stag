...
use stag_api::{
    ...
    types::ics::core::ics24_host::identifier::{ChainId, PortId},
};
use tracing::error;
...
#[function_component(CloseChannelForm)]
pub fn close_channel_form(props: &Props) -> Html {
                        ...
                        .await
                        {
                            Ok(()) => {
                                ...
                                notification.set(Some(NotificationData::success(
                                    "Successfully closed channel".to_string(),
                                )));
                            }
                            Err(err) => {
                                error!("Failed to close channel: {:?}", err);
    ...
    html! {
        <div class={classes!("border-t-2", "border-slate-400", "p-6")}>
            <h2 class={classes!("text-2xl", "pb-6", "font-bold")}>{ "Close Channel" }</h2>
...
async fn close_channel(
    ...
    match channel_type.as_str() {
        "Transfer" => {
            stag.close_channel(chain_id, &PortId::transfer(), None, memo)
                .await
        }
        "ICA" => {
            stag.close_channel(chain_id, &PortId::ica_controller(), None, memo)
                .await
        }
